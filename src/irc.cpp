...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MLCcoinTEST2\r");
            Send(hSocket, "WHO #MLCcoinTEST2\r");
        } else {
            // randomly join #MLCcoin00-#MLCcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #MLCcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MLCcoin%02d\r", channel_number).c_str());
        }
